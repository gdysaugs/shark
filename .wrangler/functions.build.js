------formdata-undici-040256966057
Content-Disposition: form-data; name="metadata"

{"main_module":"functionsWorker-0.9949046095692325.js"}
------formdata-undici-040256966057
Content-Disposition: form-data; name="functionsWorker-0.9949046095692325.js"; filename="functionsWorker-0.9949046095692325.js"
...
var corsMethods2 = "POST, OPTIONS";
var jsonResponse2 = /* @__PURE__ */ __name((body, status = 200, headers = {}) => new Response(JSON.stringify(body), {
  status,
  headers: { ...headers, "Content-Type": "application/json; charset=utf-8" }
}), "jsonResponse");
var deprecatedBody2 = {
...
var onRequestOptions2 = /* @__PURE__ */ __name(async ({ request, env }) => {
  const corsHeaders = buildCorsHeaders(request, env, corsMethods2);
  if (isCorsBlocked(request, env)) {
    return new Response(null, { status: 403, headers: corsHeaders });
  }
  return jsonResponse2(deprecatedBody2, 410, corsHeaders);
}, "onRequestOptions");
...
var corsMethods3 = "GET, POST, OPTIONS";
...
var requireGoogleUser = /* @__PURE__ */ __name(async (request, env, corsHeaders) => {
  const token = extractBearerToken(request);
  if (!token) {
    return { response: jsonResponse3({ error: "\u30ED\u30B0\u30A4\u30F3\u304C\u5FC5\u8981\u3067\u3059\u3002" }, 401, corsHeaders) };
  }
  const admin = getSupabaseAdmin(env);
  if (!admin) {
    return {
      response: jsonResponse3({ error: "SUPABASE_URL or SUPABASE_SERVICE_ROLE_KEY is not set." }, 500, corsHeaders)
    };
  }
  const { data, error } = await admin.auth.getUser(token);
  if (error || !data?.user) {
    return { response: jsonResponse3({ error: "\u8A8D\u8A3C\u306B\u5931\u6557\u3057\u307E\u3057\u305F\u3002" }, 401, corsHeaders) };
  }
  if (!isGoogleUser(data.user)) {
    return { response: jsonResponse3({ error: "Google\u30ED\u30B0\u30A4\u30F3\u306E\u307F\u5229\u7528\u3067\u304D\u307E\u3059\u3002" }, 403, corsHeaders) };
  }
  return { admin, user: data.user };
}, "requireGoogleUser");
var onRequestOptions3 = /* @__PURE__ */ __name(async ({ request, env }) => {
  const corsHeaders = buildCorsHeaders(request, env, corsMethods3);
  if (isCorsBlocked(request, env)) {
    return new Response(null, { status: 403, headers: corsHeaders });
  }
...
var onRequestGet = /* @__PURE__ */ __name(async ({ request, env }) => {
  ...
  if (error) {
    return jsonResponse3({ error: error.message }, 500, corsHeaders);
  }
  if (!ticketRow) {
    return jsonResponse3({ error: "No ticket row." }, 500, corsHeaders);
  }
  const bonus = await fetchDailyBonusState(auth.admin, ticketRow.id);
  if (bonus.error) {
    return jsonResponse3({ error: bonus.error.message }, 500, corsHeaders);
  }
...
var onRequestPost3 = /* @__PURE__ */ __name(async ({ request, env }) => {
  ...
  if (!email) {
    return jsonResponse3({ error: "\u30E1\u30FC\u30EB\u30A2\u30C9\u30EC\u30B9\u304C\u53D6\u5F97\u3067\u304D\u307E\u305B\u3093\u3002" }, 400, corsHeaders);
  }
  const { data: ticketRow, error: ticketError } = await ensureTicketRow(auth.admin, auth.user);
  if (ticketError) {
    return jsonResponse3({ error: ticketError.message }, 500, corsHeaders);
  }
  if (!ticketRow) {
    return jsonResponse3({ error: "No ticket row." }, 500, corsHeaders);
  }
  ...
  if (bonusState.error) {
    return jsonResponse3({ error: bonusState.error.message }, 500, corsHeaders);
  }
  if (!bonusState.data) {
    return jsonResponse3({ error: "daily_bonus_state row not found." }, 500, corsHeaders);
  }
  const nowMs = Date.now();
  ...
  if (claimSlot.error) {
    return jsonResponse3({ error: claimSlot.error.message }, 500, corsHeaders);
  }
  ...
  if (grant.error) {
    ...
    return jsonResponse3({ error: grant.error.message }, 500, corsHeaders);
  }
...
var DEFAULT_QWEN_EDIT_ENDPOINT = "https://api.runpod.ai/v2/h7p0hwtyzvndp5";
...
var requireAuthenticatedUser = /* @__PURE__ */ __name(async (request, env, corsHeaders) => {
  const token = extractBearerToken2(request);
  if (!token) {
    return { response: jsonResponse4({ error: "Login is required." }, 401, corsHeaders) };
  }
  ...
  const { data, error } = await admin.auth.getUser(token);
  if (error || !data?.user) {
    return { response: jsonResponse4({ error: "Authentication failed." }, 401, corsHeaders) };
  }
  return { admin, user: data.user };
...
var ensureTicketAvailable = /* @__PURE__ */ __name(async (admin, user, corsHeaders) => {
  const email = user.email;
  if (!email) {
    return { response: jsonResponse4({ error: "Email is required." }, 400, corsHeaders) };
  }
  const { data: existing, error } = await ensureTicketRow2(admin, user);
  if (error) {
    return { response: jsonResponse4({ error: error.message }, 500, corsHeaders) };
  }
  if (!existing) {
    return { response: jsonResponse4({ error: "No ticket remaining." }, 402, corsHeaders) };
  }
  if (!existing.user_id) {
...
var consumeTicket = /* @__PURE__ */ __name(async (admin, user, metadata, usageId, corsHeaders) => {
  const email = user.email;
  if (!email) {
    return { response: jsonResponse4({ error: "Email is required." }, 400, corsHeaders) };
  }
  ...
  if (rpcError) {
    ...
    if (message.includes("INVALID")) {
      return { response: jsonResponse4({ error: "Invalid ticket request." }, 400, corsHeaders) };
    }
    return { response: jsonResponse4({ error: message }, 500, corsHeaders) };
  }
...
var refundTicket = /* @__PURE__ */ __name(async (admin, user, metadata, usageId, corsHeaders) => {
  const email = user.email;
  if (!email || !usageId) {
    return { skipped: true };
  }
  ...
  if (chargeError) {
    return { response: jsonResponse4({ error: chargeError.message }, 500, corsHeaders) };
  }
  ...
  if (refundCheckError) {
    return { response: jsonResponse4({ error: refundCheckError.message }, 500, corsHeaders) };
  }
  if (existingRefund) {
  ...
  if (rpcError) {
    ...
    if (message.includes("INVALID")) {
      return { response: jsonResponse4({ error: message }, 400, corsHeaders) };
    }
    return { response: jsonResponse4({ error: message }, 500, corsHeaders) };
  }
  const result = Array.isArray(rpcData) ? rpcData[0] : rpcData;
...
var ensureUsageOwnership = /* @__PURE__ */ __name(async (admin, user, usageId, corsHeaders) => {
  const { data: chargeEvent, error: chargeError } = await admin.from("ticket_events").select("user_id, email, metadata").eq("usage_id", usageId).maybeSingle();
  if (chargeError) {
    return { response: jsonResponse4({ error: chargeError.message }, 500, corsHeaders) };
  }
  if (!chargeEvent) {
    return { response: jsonResponse4({ error: "Job not found." }, 404, corsHeaders) };
  }
  const email = user.email ?? "";
  const chargeUserId = chargeEvent.user_id ? String(chargeEvent.user_id) : "";
  const chargeEmail = chargeEvent.email ? String(chargeEvent.email) : "";
  ...
  return { ok: true, chargeEvent };
}, "ensureUsageOwnership");
var getBoundJobIdFromMetadata = /* @__PURE__ */ __name((metadata) => {
  if (!metadata || typeof metadata !== "object" || Array.isArray(metadata)) return "";
  const raw = metadata.job_id;
  return typeof raw === "string" ? raw.trim() : "";
}, "getBoundJobIdFromMetadata");
var bindUsageToJobId = /* @__PURE__ */ __name(async (admin, user, usageId, jobId, corsHeaders) => {
  const { data: chargeEvent, error: chargeError } = await admin.from("ticket_events").select("user_id, email, metadata").eq("usage_id", usageId).maybeSingle();
  if (chargeError) {
    return { response: jsonResponse4({ error: chargeError.message }, 500, corsHeaders) };
  }
  ...
  const currentMetadata = chargeEvent.metadata && typeof chargeEvent.metadata === "object" && !Array.isArray(chargeEvent.metadata) ? chargeEvent.metadata : {};
  const boundJobId = getBoundJobIdFromMetadata(currentMetadata);
  if (boundJobId && boundJobId !== jobId) {
    return { response: jsonResponse4({ error: "usage_id does not match job id." }, 409, corsHeaders) };
  }
  if (boundJobId === jobId) {
    return { ok: true };
  }
  const nextMetadata = { ...currentMetadata, job_id: jobId };
  const { error: updateError } = await admin.from("ticket_events").update({ metadata: nextMetadata }).eq("usage_id", usageId);
  if (updateError) {
    return { response: jsonResponse4({ error: updateError.message }, 500, corsHeaders) };
  }
  return { ok: true };
}, "bindUsageToJobId");
...
var isFailureStatus = /* @__PURE__ */ __name((payload) => {
...
var onRequestOptions4 = /* @__PURE__ */ __name(async ({ request, env }) => {
  const corsHeaders = buildCorsHeaders(request, env, corsMethods4);
  if (isCorsBlocked(request, env)) {
    return new Response(null, { status: 403, headers: corsHeaders });
  }
  return new Response(null, { headers: corsHeaders });
...
var onRequestGet3 = /* @__PURE__ */ __name(async ({ request, env }) => {
  ...
  try {
    ...
    if (!id) {
      return jsonResponse4({ error: "id is required." }, 400, corsHeaders);
    }
    if (!usageId) {
      return jsonResponse4({ error: "usage_id is required." }, 400, corsHeaders);
    }
    if (!env.RUNPOD_API_KEY) {
      return jsonResponse4({ error: "RUNPOD_API_KEY is not set." }, 500, corsHeaders);
    }
    ...
    if (!boundJobId) {
      return jsonResponse4({ error: "Job binding is missing. Please regenerate." }, 409, corsHeaders);
    }
    if (boundJobId !== id) {
      return jsonResponse4({ error: "Job not found." }, 404, corsHeaders);
    }
    ...
    if (ticketsLeft !== null && payload && typeof payload === "object" && !Array.isArray(payload)) {
      ...
      return jsonResponse4(payload, upstream.status, corsHeaders);
    }
...
var onRequestPost4 = /* @__PURE__ */ __name(async ({ request, env }) => {
  ...
  try {
    ...
    if (!payload) {
      return jsonResponse4({ error: "Invalid request body." }, 400, corsHeaders);
    }
    ...
    try {
      ...
      return jsonResponse4({ error: error instanceof Error ? error.message : "Failed to read image." }, 400, corsHeaders);
    }
    const subImageBase64 = subImageBase64Raw || imageBase64;
    try {
      if (imageBase64 && await isUnderageImage(imageBase64, env)) {
        return jsonResponse4({ error: UNDERAGE_BLOCK_MESSAGE }, 400, corsHeaders);
      }
    ...
    if (prompt.length > MAX_PROMPT_LENGTH) {
      return jsonResponse4({ error: "Prompt is too long." }, 400, corsHeaders);
    }
    if (negativePrompt.length > MAX_NEGATIVE_PROMPT_LENGTH) {
      return jsonResponse4({ error: "Negative prompt is too long." }, 400, corsHeaders);
    }
    ...
    if (safeInput?.workflow) {
      return jsonResponse4({ error: "workflow overrides are not allowed." }, 400, corsHeaders);
    }
    const ticketMeta = {
    ...
    if (useComfyUi) {
      ...
      if (!workflow || Object.keys(workflow).length === 0) {
        return jsonResponse4({ error: "workflow.json is empty. Export a ComfyUI API workflow." }, 500, corsHeaders);
      }
      nodeMap = getNodeMap(variant);
      const hasNodeMap = nodeMap && Object.keys(nodeMap).length > 0;
      if (!hasNodeMap) {
        return jsonResponse4({ error: "node_map.json is empty." }, 500, corsHeaders);
      }
    }
    ...
    if (useComfyUi) {
      ...
      if (hasPrimaryImageNode && !imageBase64) {
        return jsonResponse4({ error: "Image is required for this workflow." }, 400, corsHeaders);
      }
      const secondaryImageBase64 = subImageBase64Raw || imageBase64;
      if (hasSecondaryImageNode && !secondaryImageBase64) {
        return jsonResponse4({ error: "Second image is required for this workflow." }, 400, corsHeaders);
      }
      ...
      const jobId2 = extractJobId(upstreamPayload2);
      if (jobId2) {
        const binding = await bindUsageToJobId(auth.admin, auth.user, usageId, String(jobId2), corsHeaders);
      ...
      if (!upstreamPayload2 || typeof upstreamPayload2 !== "object" || Array.isArray(upstreamPayload2)) {
        ...
        return jsonResponse4({ error: "Upstream response is invalid.", usage_id: usageId, ticketsLeft }, 502, corsHeaders);
      }
      const isFailure2 = !upstream2.ok || isFailureStatus(upstreamPayload2) || hasOutputError(upstreamPayload2);
      if (isFailure2) {
        const refundResult = await refundTicket(
          auth.admin,
          auth.user,
      ...
      return jsonResponse4(upstreamPayload2, upstream2.status, corsHeaders);
    }
    const runpodInput = {
    ...
    return jsonResponse4(upstreamPayload, upstream.status, corsHeaders);
  } catch (error) {
    return jsonResponse4(
      {
...
var qwen_sparkart_workflow_default = {
  ...
  "10": {
    ...
    _meta: {
      title: "Multiple Angles LoRA"
    }
  }
};

// api/qwen_sparkart.ts
var corsMethods5 = "POST, GET, OPTIONS";
var DEFAULT_QWEN_EDIT_ENDPOINT2 = "https://api.runpod.ai/v2/h7p0hwtyzvndp5";
...
var INTERNAL_ERROR_MESSAGE = "\u30A8\u30E9\u30FC\u3067\u3059\u3002\u3084\u308A\u76F4\u3057\u3066\u304F\u3060\u3055\u3044\u3002";
var internalErrorResponse = /* @__PURE__ */ __name((corsHeaders) => jsonResponse5({ error: INTERNAL_ERROR_MESSAGE }, 500, corsHeaders), "internalErrorResponse");
...
var requireAuthenticatedUser2 = /* @__PURE__ */ __name(async (request, env, corsHeaders) => {
  const token = extractBearerToken3(request);
  if (!token) {
    return { response: jsonResponse5({ error: "Login is required." }, 401, corsHeaders) };
  }
  ...
  const { data, error } = await admin.auth.getUser(token);
  if (error || !data?.user) {
    return { response: jsonResponse5({ error: "Authentication failed." }, 401, corsHeaders) };
  }
  return { admin, user: data.user };
}, "requireAuthenticatedUser");
...
var ensureTicketAvailable2 = /* @__PURE__ */ __name(async (admin, user, requiredTickets = 1, corsHeaders = {}) => {
  const email = user.email;
  if (!email) {
    return { response: jsonResponse5({ error: "Email not available." }, 400, corsHeaders) };
  }
  ...
  if (!existing) {
    return { response: jsonResponse5({ error: "No tickets remaining." }, 402, corsHeaders) };
  }
  if (!existing.user_id) {
    await admin.from("user_tickets").update({ user_id: user.id }).eq("id", existing.id);
  }
...
var consumeTicket2 = /* @__PURE__ */ __name(async (admin, user, metadata, usageId, ticketCost = 1, corsHeaders = {}) => {
  ...
  if (!existing) {
    return { response: jsonResponse5({ error: "No tickets available." }, 402, corsHeaders) };
  }
  if (!existing.user_id) {
    await admin.from("user_tickets").update({ user_id: user.id }).eq("id", existing.id);
  }
  ...
  if (rpcError) {
    ...
    if (message.includes("INVALID")) {
      return { response: jsonResponse5({ error: "Invalid ticket request." }, 400, corsHeaders) };
    }
    return { response: internalErrorResponse(corsHeaders) };
  }
  const result = Array.isArray(rpcData) ? rpcData[0] : rpcData;
...
var refundTicket2 = /* @__PURE__ */ __name(async (admin, user, metadata, usageId, ticketAmount = 1, corsHeaders = {}) => {
  ...
  if (rpcError) {
    ...
    if (message.includes("INVALID")) {
      return { response: jsonResponse5({ error: message }, 400, corsHeaders) };
    }
    return { response: internalErrorResponse(corsHeaders) };
  }
  const result = Array.isArray(rpcData) ? rpcData[0] : rpcData;
...
var ensureUsageOwnership2 = /* @__PURE__ */ __name(async (admin, user, usageId, corsHeaders) => {
  ...
  if (!chargeEvent) {
    return { response: jsonResponse5({ error: "Job not found." }, 404, corsHeaders) };
  }
  const email = user.email ?? "";
  const chargeUserId = chargeEvent.user_id ? String(chargeEvent.user_id) : "";
  const chargeEmail = chargeEvent.email ? String(chargeEvent.email) : "";
...
var isMissingMultiAngleLoraError = /* @__PURE__ */ __name((payload) => {
  const text = JSON.stringify(payload ?? "").toLowerCase();
  return text.includes("prompt_outputs_failed_validation") && text.includes("lora_name") && text.includes("qwen-image-edit-2511-multiple-angles-lora.safetensors") && text.includes("not in []");
}, "isMissingMultiAngleLoraError");
var isFailureStatus2 = /* @__PURE__ */ __name((status) => {
...
var onRequestOptions5 = /* @__PURE__ */ __name(async ({ request, env }) => {
  const corsHeaders = buildCorsHeaders(request, env, corsMethods5);
  if (isCorsBlocked(request, env)) {
    return new Response(null, { status: 403, headers: corsHeaders });
  }
  return new Response(null, { headers: corsHeaders });
...
var onRequestGet4 = /* @__PURE__ */ __name(async ({ request, env }) => {
  ...
  try {
    const auth = await requireAuthenticatedUser2(request, env, corsHeaders);
    if ("response" in auth) {
      return auth.response;
    }
    const endpoint = resolveEndpoint2(env);
    if (!env.RUNPOD_API_KEY) {
      return jsonResponse5({ error: "RUNPOD_API_KEY is not set." }, 500, corsHeaders);
    }
    const url = new URL(request.url);
    const id = url.searchParams.get("id");
    const usageId = url.searchParams.get("usage_id") ?? url.searchParams.get("usageId") ?? "";
    if (!id) {
      return jsonResponse5({ error: "id is required." }, 400, corsHeaders);
    }
    if (!usageId) {
      return jsonResponse5({ error: "usage_id is required." }, 400, corsHeaders);
    }
    const expectedUsageId = `qwen_sparkart:${id}`;
    if (usageId !== expectedUsageId) {
      return jsonResponse5({ error: "usage_id and id do not match." }, 400, corsHeaders);
    }
    const ownership = await ensureUsageOwnership2(auth.admin, auth.user, usageId, corsHeaders);
    if ("response" in ownership) {
      return ownership.response;
    }
    ...
    if (payload && (isFailureStatus2(payload?.status ?? payload?.state ?? "") || pickErrorMessage(payload))) {
      const ticketMeta = {
        job_id: id,
        status: payload?.status ?? payload?.state ?? null,
        source: "status",
      ...
      const refundResult = await refundTicket2(auth.admin, auth.user, ticketMeta, usageId, 1, corsHeaders);
    ...
    if (payload && typeof payload === "object" && !Array.isArray(payload)) {
      ...
      return jsonResponse5(payload, upstream.status, corsHeaders);
    }
    return new Response(raw, {
      status: upstream.status,
      headers: { ...corsHeaders, "Content-Type": "application/json" }
    ...
    return jsonResponse5({ error: "Qwen status request failed." }, 502, corsHeaders);
  }
}, "onRequestGet");
var onRequestPost5 = /* @__PURE__ */ __name(async ({ request, env }) => {
  ...
  try {
    ...
    const payload = await request.json().catch(() => null);
    if (!payload) {
      return jsonResponse5({ error: "Invalid request body." }, 400, corsHeaders);
    }
    const input = payload.input ?? payload;
    ...
    if (prompt.length > MAX_PROMPT_LENGTH2) {
      return jsonResponse5({ error: `prompt is too long (max ${MAX_PROMPT_LENGTH2}).` }, 400, corsHeaders);
    }
    if (promptIsEmpty && !multiAngleEnabled) {
      return jsonResponse5({ error: `prompt is required (max ${MAX_PROMPT_LENGTH2}).` }, 400, corsHeaders);
    }
    if (negativePrompt.length > MAX_PROMPT_LENGTH2) {
      return jsonResponse5({ error: `negative_prompt is too long (max ${MAX_PROMPT_LENGTH2}).` }, 400, corsHeaders);
    }
    let imageBase64 = "";
    try {
      ...
      return jsonResponse5({ error: error instanceof Error ? error.message : "Invalid image." }, 400, corsHeaders);
    }
    ...
    const ticketCheck = await ensureTicketAvailable2(auth.admin, auth.user, 1, corsHeaders);
    if ("response" in ticketCheck) {
      return ticketCheck.response;
    }
    ...
    let ticketsLeft = null;
    let usageId = null;
    const shouldCharge = upstream.ok && !isFailureStatus2(upstreamStatus) && !upstreamError;
    if (shouldCharge) {
      const jobId = extractJobId2(parsed);
      ...
      const chargeResult = await consumeTicket2(auth.admin, auth.user, ticketMeta, usageId, 1, corsHeaders);
    ...
    if (upstream.ok && !extractJobId2(parsed) && Array.isArray(parsed?.output?.images)) {
      return jsonResponse5(parsed, 200, corsHeaders);
    }
    return jsonResponse5(parsed, upstream.status, corsHeaders);
  } catch (error) {
    return jsonResponse5({ error: "Qwen request failed." }, 502, corsHeaders);
  }
}, "onRequestPost");

// _shared/sigv4.ts
var toHex2 = /* @__PURE__ */ __name((bytes) => [...new Uint8Array(bytes)].map((b) => b.toString(16).padStart(2, "0")).join(""), "toHex");
...
`;
  ...
  const signature = toHex2(await hmacSha256(kSigning, stringToSign));
...
var requireAuthenticatedUser3 = /* @__PURE__ */ __name(async (request, env, corsHeaders) => {
  const token = extractBearerToken4(request);
  if (!token) {
    return { response: jsonResponse6({ error: "\u30ED\u30B0\u30A4\u30F3\u304C\u5FC5\u8981\u3067\u3059\u3002" }, 401, corsHeaders) };
  }
  const admin = getSupabaseAdmin4(env);
  if (!admin) {
    return { response: jsonResponse6({ error: "SUPABASE_URL \u307E\u305F\u306F SUPABASE_SERVICE_ROLE_KEY \u304C\u672A\u8A2D\u5B9A\u3067\u3059\u3002" }, 500, corsHeaders) };
  }
  const { data, error } = await admin.auth.getUser(token);
  if (error || !data?.user) {
    return { response: jsonResponse6({ error: "\u8A8D\u8A3C\u306B\u5931\u6557\u3057\u307E\u3057\u305F\u3002" }, 401, corsHeaders) };
  }
...
var onRequestOptions6 = /* @__PURE__ */ __name(async ({ request, env }) => {
  const corsHeaders = buildCorsHeaders(request, env, corsMethods6);
  if (isCorsBlocked(request, env)) {
    return new Response(null, { status: 403, headers: corsHeaders });
  }
...
var onRequestPost6 = /* @__PURE__ */ __name(async ({ request, env }) => {
  ...
  const auth = await requireAuthenticatedUser3(request, env, corsHeaders);
  if ("response" in auth) return auth.response;
  try {
    const body = await request.json().catch(() => null);
    if (!body || typeof body !== "object") {
      return jsonResponse6({ error: "Invalid request body." }, 400, corsHeaders);
    }
    const contentType = String(body.contentType || "").trim();
    if (!contentType) {
      return jsonResponse6({ error: "contentType is required." }, 400, corsHeaders);
    }
    ...
    return jsonResponse6({ error: "R2\u7F72\u540DURL\u306E\u751F\u6210\u306B\u5931\u6557\u3057\u307E\u3057\u305F\u3002", message }, 500, corsHeaders);
  }
}, "onRequestPost");

// api/tickets.ts
var SIGNUP_TICKET_GRANT4 = 5;
var corsMethods7 = "GET, OPTIONS";
...
var requireAuthenticatedUser4 = /* @__PURE__ */ __name(async (request, env, corsHeaders) => {
  const token = extractBearerToken5(request);
  if (!token) {
    return { response: jsonResponse7({ error: "\u30ED\u30B0\u30A4\u30F3\u304C\u5FC5\u8981\u3067\u3059\u3002" }, 401, corsHeaders) };
  }
  const admin = getSupabaseAdmin5(env);
  if (!admin) {
    return { response: jsonResponse7({ error: "SUPABASE_URL or SUPABASE_SERVICE_ROLE_KEY is not set." }, 500, corsHeaders) };
  }
  const { data, error } = await admin.auth.getUser(token);
  if (error || !data?.user) {
    return { response: jsonResponse7({ error: "\u8A8D\u8A3C\u306B\u5931\u6557\u3057\u307E\u3057\u305F\u3002" }, 401, corsHeaders) };
  }
  return { admin, user: data.user };
}, "requireAuthenticatedUser");
var onRequestOptions7 = /* @__PURE__ */ __name(async ({ request, env }) => {
  const corsHeaders = buildCorsHeaders(request, env, corsMethods7);
  if (isCorsBlocked(request, env)) {
    return new Response(null, { status: 403, headers: corsHeaders });
  }
  return new Response(null, { headers: corsHeaders });
...
var onRequestGet5 = /* @__PURE__ */ __name(async ({ request, env }) => {
  ...
  const auth = await requireAuthenticatedUser4(request, env, corsHeaders);
  if ("response" in auth) {
    return auth.response;
  }
  const email = auth.user.email;
  if (!email) {
    return jsonResponse7({ error: "\u30E1\u30FC\u30EB\u30A2\u30C9\u30EC\u30B9\u304C\u53D6\u5F97\u3067\u304D\u307E\u305B\u3093\u3002" }, 400, corsHeaders);
  }
  const { data, error } = await ensureTicketRow4(auth.admin, auth.user);
  if (error) {
    return jsonResponse7({ error: error.message }, 500, corsHeaders);
  }
  return jsonResponse7({ tickets: data?.tickets ?? 0, hasRecord: Boolean(data) }, 200, corsHeaders);
}, "onRequestGet");
...
var INTERNAL_ERROR_MESSAGE2 = "\u30A8\u30E9\u30FC\u3067\u3059\u3002\u3084\u308A\u76F4\u3057\u3066\u304F\u3060\u3055\u3044\u3002";
var internalErrorResponse2 = /* @__PURE__ */ __name((corsHeaders) => jsonResponse8({ error: INTERNAL_ERROR_MESSAGE2 }, 500, corsHeaders), "internalErrorResponse");
...
var requireAuthenticatedUser5 = /* @__PURE__ */ __name(async (request, env, corsHeaders) => {
  const token = extractBearerToken6(request);
  if (!token) {
    return { response: jsonResponse8({ error: "\u30ED\u30B0\u30A4\u30F3\u304C\u5FC5\u8981\u3067\u3059\u3002" }, 401, corsHeaders) };
  }
  ...
  const { data, error } = await admin.auth.getUser(token);
  if (error || !data?.user) {
    return { response: jsonResponse8({ error: "\u8A8D\u8A3C\u306B\u5931\u6557\u3057\u307E\u3057\u305F\u3002" }, 401, corsHeaders) };
  }
  return { admin, user: data.user };
}, "requireAuthenticatedUser");
...
var ensureTicketAvailable3 = /* @__PURE__ */ __name(async (admin, user, requiredTickets = 1, corsHeaders = {}) => {
  const email = user.email;
  if (!email) {
    return { response: jsonResponse8({ error: "Email not available." }, 400, corsHeaders) };
  }
  const { data: existing, error } = await ensureTicketRow5(admin, user);
  if (error) {
    return { response: internalErrorResponse2(corsHeaders) };
  }
  if (!existing) {
    return { response: jsonResponse8({ error: "No tickets available." }, 402, corsHeaders) };
  }
  if (!existing.user_id) {
    await admin.from("user_tickets").update({ user_id: user.id }).eq("id", existing.id);
  }
  if (existing.tickets < requiredTickets) {
    return { response: jsonResponse8({ error: "No tickets remaining." }, 402, corsHeaders) };
  }
  return { existing };
}, "ensureTicketAvailable");
var consumeTicket3 = /* @__PURE__ */ __name(async (admin, user, metadata, usageId, ticketCost = 1, corsHeaders = {}) => {
  const cost = Math.max(1, Math.floor(ticketCost));
  const email = user.email;
  if (!email) {
    return { response: jsonResponse8({ error: "Email not available." }, 400, corsHeaders) };
  ...
  if (rpcError) {
    ...
    if (message.includes("INVALID")) {
      return { response: jsonResponse8({ error: "Invalid ticket request." }, 400, corsHeaders) };
    }
    return { response: internalErrorResponse2(corsHeaders) };
  }
  const result = Array.isArray(rpcData) ? rpcData[0] : rpcData;
...
var refundTicket3 = /* @__PURE__ */ __name(async (admin, user, metadata, usageId, ticketCost = 1, corsHeaders = {}) => {
...
var ensureUsageOwnership3 = /* @__PURE__ */ __name(async (admin, user, usageId, corsHeaders) => {
  const { data: chargeEvent, error: chargeError } = await admin.from("ticket_events").select("user_id, email").eq("usage_id", usageId).maybeSingle();
  if (chargeError) {
    return { response: internalErrorResponse2(corsHeaders) };
  }
  if (!chargeEvent) {
    return { response: jsonResponse8({ error: "Job not found." }, 404, corsHeaders) };
  }
  const email = user.email ?? "";
  const chargeUserId = chargeEvent.user_id ? String(chargeEvent.user_id) : "";
  const chargeEmail = chargeEvent.email ? String(chargeEvent.email) : "";
...
var onRequestOptions8 = /* @__PURE__ */ __name(async ({ request, env }) => {
  const corsHeaders = buildCorsHeaders(request, env, corsMethods8);
  if (isCorsBlocked(request, env)) {
    return new Response(null, { status: 403, headers: corsHeaders });
  }
  return new Response(null, { headers: corsHeaders });
...
var onRequestGet6 = /* @__PURE__ */ __name(async ({ request, env }) => {
  ...
  const auth = await requireAuthenticatedUser5(request, env, corsHeaders);
  if ("response" in auth) {
    return auth.response;
  }
  const url = new URL(request.url);
  const id = url.searchParams.get("id");
  if (!id) {
    return jsonResponse8({ error: "id\u304C\u5FC5\u8981\u3067\u3059\u3002" }, 400, corsHeaders);
  }
  if (!env.RUNPOD_API_KEY) {
    return jsonResponse8({ error: "RUNPOD_API_KEY is not set." }, 500, corsHeaders);
  }
  const endpoint = resolveEndpoint3(env);
  if (!endpoint) {
    return jsonResponse8({ error: "RUNPOD_WAN_ENDPOINT_URL is not set." }, 500, corsHeaders);
  }
  const ownershipUsageId = `wan:${id}`;
  const ownership = await ensureUsageOwnership3(auth.admin, auth.user, ownershipUsageId, corsHeaders);
  if ("response" in ownership) {
    return ownership.response;
  }
  const upstream = await fetch(`${endpoint}/status/${encodeURIComponent(id)}`, {
  ...
  if (payload && shouldConsumeTicket(payload)) {
    ...
    const result = await consumeTicket3(auth.admin, auth.user, ticketMeta, usageId, VIDEO_TICKET_COST, corsHeaders);
  ...
  if (payload && (isFailureStatus3(payload) || hasOutputError2(payload))) {
    ...
    const refundResult = await refundTicket3(auth.admin, auth.user, refundMeta, usageId, VIDEO_TICKET_COST, corsHeaders);
    if ("response" in refundResult) {
      return refundResult.response;
    }
    const nextTickets = Number(refundResult.ticketsLeft);
  ...
  if (ticketsLeft !== null && payload && typeof payload === "object" && !Array.isArray(payload)) {
    payload.ticketsLeft = ticketsLeft;
    return jsonResponse8(payload, upstream.status, corsHeaders);
  }
  return new Response(raw, {
    status: upstream.status,
    headers: { ...corsHeaders, "Content-Type": "application/json" }
...
var onRequestPost7 = /* @__PURE__ */ __name(async ({ request, env }) => {
  ...
  const payload = await request.json().catch(() => null);
  if (!payload) {
    return jsonResponse8({ error: "Invalid request body." }, 400, corsHeaders);
  }
  const input = payload.input ?? payload;
  if (input?.workflow) {
    return jsonResponse8({ error: "workflow overrides are not allowed." }, 400, corsHeaders);
  }
  const mode = String(input?.mode ?? "i2v").toLowerCase();
  if (mode !== "i2v" && mode !== "t2v") {
    return jsonResponse8({ error: 'mode must be "i2v" or "t2v".' }, 400, corsHeaders);
  }
  const isT2V = mode === "t2v";
  const imageValue = input?.image_base64 ?? input?.image ?? input?.image_url;
  if (!isT2V && !imageValue) {
    return jsonResponse8({ error: "i2v\u306B\u306F\u753B\u50CF\u304C\u5FC5\u8981\u3067\u3059\u3002" }, 400, corsHeaders);
  }
  let imageBase64 = "";
  try {
    if (imageValue) {
  ...
  if (!isT2V && !imageBase64) {
    return jsonResponse8({ error: "image is empty." }, 400, corsHeaders);
  }
  if (!isT2V) {
    try {
      if (await isUnderageImage(imageBase64, env)) {
        return jsonResponse8({ error: UNDERAGE_BLOCK_MESSAGE2 }, 400, corsHeaders);
      }
    } catch (error) {
      return jsonResponse8(
  ...
  if (prompt.length > MAX_PROMPT_LENGTH3) {
    return jsonResponse8({ error: "Prompt is too long." }, 400, corsHeaders);
  }
  if (negativePrompt.length > MAX_NEGATIVE_PROMPT_LENGTH2) {
    return jsonResponse8({ error: "Negative prompt is too long." }, 400, corsHeaders);
  }
  if (!Number.isFinite(cfg) || cfg < MIN_CFG || cfg > MAX_CFG) {
    return jsonResponse8({ error: `cfg must be between ${MIN_CFG} and ${MAX_CFG}.` }, 400, corsHeaders);
  }
  ...
  const ticketCheck = await ensureTicketAvailable3(auth.admin, auth.user, VIDEO_TICKET_COST, corsHeaders);
  if ("response" in ticketCheck) {
    return ticketCheck.response;
  }
  ...
  if (!workflow || Object.keys(workflow).length === 0) {
    return jsonResponse8({ error: "wan workflow is empty. Export a ComfyUI API workflow." }, 500, corsHeaders);
  }
  const nodeMap = await getNodeMap2(isT2V ? "t2v" : "i2v").catch(() => null);
  const hasNodeMap = nodeMap && Object.keys(nodeMap).length > 0;
  if (!hasNodeMap) {
    return jsonResponse8({ error: "wan node map is empty." }, 500, corsHeaders);
  }
  const nodeValues = {
  ...
  if (shouldCharge && jobId) {
    ...
    const result = await consumeTicket3(auth.admin, auth.user, ticketMetaWithJob, usageId, VIDEO_TICKET_COST, corsHeaders);
    if ("response" in result) {
      return result.response;
    }
    const nextTickets = Number(result.ticketsLeft);
  ...
  if (ticketsLeft !== null && upstreamPayload && typeof upstreamPayload === "object" && !Array.isArray(upstreamPayload)) {
    upstreamPayload.ticketsLeft = ticketsLeft;
    return jsonResponse8(upstreamPayload, upstream.status, corsHeaders);
  }
  return new Response(raw, {
    status: upstream.status,
    headers: { ...corsHeaders, "Content-Type": "application/json" }
...
// api/wan-rapid-workflow-i2v.json
var wan_rapid_workflow_i2v_default = {
  "97": {
    class_type: "CLIPLoader",
    inputs: {
      clip_name: "umt5_xxl_fp8_e4m3fn_scaled.safetensors",
  ...
  "99": {
    ...
    inputs: {
      ...
      frame_rate: 12,
      loop_count: 0,
      filename_prefix: "video/sparkart_i2v_smoothmix",
      format: "video/h264-mp4",
      pix_fmt: "yuv420p",
      crf: 14,
      save_metadata: true,
...
// api/wan-rapid-node-map-i2v.json
var wan_rapid_node_map_i2v_default = {
  image: { id: "106", input: "image" },
  prompt: { id: "6", input: "text" },
  negative_prompt: { id: "7", input: "text" },
  seed: [
...
// api/wan_remix.ts
var corsMethods9 = "POST, GET, OPTIONS";
var jsonResponse9 = /* @__PURE__ */ __name((body, status = 200, headers = {}) => new Response(JSON.stringify(body), {
  status,
  headers: { ...headers, "Content-Type": "application/json" }
}), "jsonResponse");
var INTERNAL_ERROR_MESSAGE3 = "\u30A8\u30E9\u30FC\u3067\u3059\u3002\u3084\u308A\u76F4\u3057\u3066\u304F\u3060\u3055\u3044\u3002";
var internalErrorResponse3 = /* @__PURE__ */ __name((corsHeaders) => jsonResponse9({ error: INTERNAL_ERROR_MESSAGE3 }, 500, corsHeaders), "internalErrorResponse");
...
var LOW_QUALITY_TICKET_COST = 1;
var MEDIUM_QUALITY_TICKET_COST = 2;
var HIGH_QUALITY_TICKET_COST = 3;
var MAX_IMAGE_BYTES4 = 10 * 1024 * 1024;
var MAX_PROMPT_LENGTH4 = 1e3;
var MAX_NEGATIVE_PROMPT_LENGTH3 = 1e3;
var FIXED_STEPS3 = 4;
var RAPID_I2V_FIXED_STEPS = 6;
var MIN_DIMENSION4 = 256;
var MAX_DIMENSION4 = 3e3;
var MIN_CFG2 = 0;
var MAX_CFG2 = 10;
var LOW_QUALITY_FPS = 8;
var MEDIUM_QUALITY_FPS = 10;
var HIGH_QUALITY_FPS = 12;
var ALLOWED_FPS = [LOW_QUALITY_FPS, MEDIUM_QUALITY_FPS, HIGH_QUALITY_FPS];
var DEFAULT_FPS = MEDIUM_QUALITY_FPS;
...
var resolveWorkflowFlavor = /* @__PURE__ */ __name((request) => {
  try {
  ...
  return "remix";
}, "resolveWorkflowFlavor");
var getWorkflowTemplate3 = /* @__PURE__ */ __name(async (mode, flavor) => {
  if (mode === "t2v") return wan_remix_workflow_t2v_default;
  return flavor === "rapid" ? wan_rapid_workflow_i2v_default : wan_remix_workflow_i2v_default;
}, "getWorkflowTemplate");
var getNodeMap3 = /* @__PURE__ */ __name(async (mode, flavor) => {
  if (mode === "t2v") return wan_remix_node_map_t2v_default;
  return flavor === "rapid" ? wan_rapid_node_map_i2v_default : wan_remix_node_map_i2v_default;
}, "getNodeMap");
...
var requireAuthenticatedUser6 = /* @__PURE__ */ __name(async (request, env, corsHeaders) => {
  const token = extractBearerToken7(request);
  if (!token) {
    return { response: jsonResponse9({ error: "\u30ED\u30B0\u30A4\u30F3\u304C\u5FC5\u8981\u3067\u3059\u3002" }, 401, corsHeaders) };
  }
  ...
  const { data, error } = await admin.auth.getUser(token);
  if (error || !data?.user) {
    return { response: jsonResponse9({ error: "\u8A8D\u8A3C\u306B\u5931\u6557\u3057\u307E\u3057\u305F\u3002" }, 401, corsHeaders) };
  }
  return { admin, user: data.user };
}, "requireAuthenticatedUser");
...
var ensureTicketAvailable4 = /* @__PURE__ */ __name(async (admin, user, requiredTickets = 1, corsHeaders = {}) => {
  const email = user.email;
  if (!email) {
    return { response: jsonResponse9({ error: "Email not available." }, 400, corsHeaders) };
  }
  const { data: existing, error } = await ensureTicketRow6(admin, user);
  if (error) {
    return { response: internalErrorResponse3(corsHeaders) };
  }
  if (!existing) {
    return { response: jsonResponse9({ error: "No tickets available." }, 402, corsHeaders) };
  }
  if (!existing.user_id) {
    await admin.from("user_tickets").update({ user_id: user.id }).eq("id", existing.id);
  }
  if (existing.tickets < requiredTickets) {
    return { response: jsonResponse9({ error: "No tickets remaining." }, 402, corsHeaders) };
  }
  return { existing };
}, "ensureTicketAvailable");
var consumeTicket4 = /* @__PURE__ */ __name(async (admin, user, metadata, usageId, ticketCost = 1, corsHeaders = {}) => {
  const cost = Math.max(1, Math.floor(ticketCost));
  const email = user.email;
  if (!email) {
    return { response: jsonResponse9({ error: "Email not available." }, 400, corsHeaders) };
  ...
  if (rpcError) {
    ...
    if (message.includes("INVALID")) {
      return { response: jsonResponse9({ error: "Invalid ticket request." }, 400, corsHeaders) };
    }
    return { response: internalErrorResponse3(corsHeaders) };
  }
  const result = Array.isArray(rpcData) ? rpcData[0] : rpcData;
...
var refundTicket4 = /* @__PURE__ */ __name(async (admin, user, metadata, usageId, ticketCost = 1, corsHeaders = {}) => {
  const refundAmount = Math.max(1, Math.floor(ticketCost));
  const email = user.email;
  if (!email || !usageId) {
    return { skipped: true };
...
var ensureUsageOwnership4 = /* @__PURE__ */ __name(async (admin, user, usageId, corsHeaders) => {
  const { data: chargeEvent, error: chargeError } = await admin.from("ticket_events").select("user_id, email").eq("usage_id", usageId).maybeSingle();
  if (chargeError) {
    return { response: internalErrorResponse3(corsHeaders) };
  }
  if (!chargeEvent) {
    return { response: jsonResponse9({ error: "Job not found." }, 404, corsHeaders) };
  }
  const email = user.email ?? "";
  const chargeUserId = chargeEvent.user_id ? String(chargeEvent.user_id) : "";
  const chargeEmail = chargeEvent.email ? String(chargeEvent.email) : "";
...
var extractJobId4 = /* @__PURE__ */ __name((payload) => payload?.id || payload?.jobId || payload?.job_id || payload?.output?.id, "extractJobId");
...
var extractFpsFromPayload = /* @__PURE__ */ __name((payload) => {
  const candidates = [
    payload?.input?.fps,
    payload?.fps,
...
var onRequestOptions9 = /* @__PURE__ */ __name(async ({ request, env }) => {
  const corsHeaders = buildCorsHeaders(request, env, corsMethods9);
  if (isCorsBlocked(request, env)) {
    return new Response(null, { status: 403, headers: corsHeaders });
  }
  return new Response(null, { headers: corsHeaders });
...
var onRequestGet7 = /* @__PURE__ */ __name(async ({ request, env }) => {
  ...
  const auth = await requireAuthenticatedUser6(request, env, corsHeaders);
  if ("response" in auth) {
    return auth.response;
  }
  const url = new URL(request.url);
  const id = url.searchParams.get("id");
  if (!id) {
    return jsonResponse9({ error: "id\u304C\u5FC5\u8981\u3067\u3059\u3002" }, 400, corsHeaders);
  }
  if (!env.RUNPOD_API_KEY) {
    return jsonResponse9({ error: "RUNPOD_API_KEY is not set." }, 500, corsHeaders);
  }
  const endpoint = resolveEndpoint4(env);
  if (!endpoint) {
    return jsonResponse9({ error: "RUNPOD_WAN_REMIX_ENDPOINT_URL is not set." }, 500, corsHeaders);
  }
  const ownershipUsageId = `wan_remix:${id}`;
  const ownership = await ensureUsageOwnership4(auth.admin, auth.user, ownershipUsageId, corsHeaders);
  if ("response" in ownership) {
    return ownership.response;
  }
  let upstream;
  try {
    ...
    if (isTimeout) {
      return jsonResponse9({ id, status: "IN_PROGRESS", state: "IN_PROGRESS" }, 200, corsHeaders);
    }
    return jsonResponse9({ error: "RunPod status check failed." }, 502, corsHeaders);
  }
  const raw = await upstream.text();
  let payload = null;
  let ticketsLeft = null;
  ...
  if (payload && shouldConsumeTicket2(payload)) {
    ...
    const result = await consumeTicket4(auth.admin, auth.user, ticketMeta, usageId, ticketCost, corsHeaders);
    if ("response" in result) {
      return result.response;
    }
    const nextTickets = Number(result.ticketsLeft);
  ...
  if (payload && (isFailureStatus4(payload) || hasOutputError3(payload))) {
    ...
    const refundResult = await refundTicket4(auth.admin, auth.user, refundMeta, usageId, ticketCost, corsHeaders);
    if ("response" in refundResult) {
      return refundResult.response;
    }
    const nextTickets = Number(refundResult.ticketsLeft);
  ...
  if (ticketsLeft !== null && payload && typeof payload === "object" && !Array.isArray(payload)) {
    payload.ticketsLeft = ticketsLeft;
    return jsonResponse9(payload, upstream.status, corsHeaders);
  }
  return new Response(raw, {
    status: upstream.status,
    headers: { ...corsHeaders, "Content-Type": "application/json" }
...
var onRequestPost8 = /* @__PURE__ */ __name(async ({ request, env }) => {
  ...
  const payload = await request.json().catch(() => null);
  if (!payload) {
    return jsonResponse9({ error: "Invalid request body." }, 400, corsHeaders);
  }
  const input = payload.input ?? payload;
  if (input?.workflow) {
    return jsonResponse9({ error: "workflow overrides are not allowed." }, 400, corsHeaders);
  }
  const mode = String(input?.mode ?? "i2v").toLowerCase();
  if (mode !== "i2v" && mode !== "t2v") {
    return jsonResponse9({ error: 'mode must be "i2v" or "t2v".' }, 400, corsHeaders);
  }
  const isT2V = mode === "t2v";
  const workflowFlavor = resolveWorkflowFlavor(request);
  if (workflowFlavor === "rapid" && isT2V) {
    return jsonResponse9({ error: "wan-rapid supports i2v only." }, 400, corsHeaders);
  }
  const imageValue = input?.image_base64 ?? input?.image ?? input?.image_url;
  if (!isT2V && !imageValue) {
    return jsonResponse9({ error: "i2v\u306B\u306F\u753B\u50CF\u304C\u5FC5\u8981\u3067\u3059\u3002" }, 400, corsHeaders);
  }
  let imageBase64 = "";
  try {
    if (imageValue) {
  ...
  if (!isT2V && !imageBase64) {
    return jsonResponse9({ error: "image is empty." }, 400, corsHeaders);
  }
  if (!isT2V) {
    try {
      if (await isUnderageImage(imageBase64, env)) {
        return jsonResponse9({ error: UNDERAGE_BLOCK_MESSAGE3 }, 400, corsHeaders);
      }
    } catch (error) {
      return jsonResponse9(
        { error: "Age verification failed." },
  ...
  const prompt = String(input?.prompt ?? input?.text ?? "");
  const negativePrompt = String(input?.negative_prompt ?? input?.negative ?? "");
  const steps = !isT2V && workflowFlavor === "rapid" ? RAPID_I2V_FIXED_STEPS : FIXED_STEPS3;
  const cfg = 1;
  ...
  const numFrames = !isT2V && workflowFlavor === "rapid" ? fps * seconds + 1 : fps * seconds;
  const seed = input?.randomize_seed ? Math.floor(Math.random() * 2147483647) : Number(input?.seed ?? 0);
  if (prompt.length > MAX_PROMPT_LENGTH4) {
    return jsonResponse9({ error: "Prompt is too long." }, 400, corsHeaders);
  }
  if (negativePrompt.length > MAX_NEGATIVE_PROMPT_LENGTH3) {
    return jsonResponse9({ error: "Negative prompt is too long." }, 400, corsHeaders);
  }
  if (!Number.isFinite(cfg) || cfg < MIN_CFG2 || cfg > MAX_CFG2) {
    return jsonResponse9({ error: `cfg must be between ${MIN_CFG2} and ${MAX_CFG2}.` }, 400, corsHeaders);
  }
  ...
  const totalSteps = Math.max(1, Math.floor(steps));
  const splitStep = !isT2V && workflowFlavor === "rapid" ? Math.max(1, Math.floor(totalSteps / 3)) : Math.max(1, Math.floor(totalSteps / 2));
  const ticketMeta = {
    prompt_length: prompt.length,
    width,
    height,
  ...
  const ticketCheck = await ensureTicketAvailable4(auth.admin, auth.user, ticketCost, corsHeaders);
  if ("response" in ticketCheck) {
    return ticketCheck.response;
  }
  const imageName = String(input?.image_name ?? "input.png");
  const workflow = clone4(await getWorkflowTemplate3(isT2V ? "t2v" : "i2v", workflowFlavor));
  if (!workflow || Object.keys(workflow).length === 0) {
    return jsonResponse9({ error: "wan workflow is empty. Export a ComfyUI API workflow." }, 500, corsHeaders);
  }
  const nodeMap = await getNodeMap3(isT2V ? "t2v" : "i2v", workflowFlavor).catch(() => null);
  const hasNodeMap = nodeMap && Object.keys(nodeMap).length > 0;
  if (!hasNodeMap) {
    return jsonResponse9({ error: "wan node map is empty." }, 500, corsHeaders);
  }
  const nodeValues = {
    image: imageBase64 ? imageName : void 0,
    prompt,
  ...
  const jobId = extractJobId4(upstreamPayload);
  ...
  if (shouldCharge && jobId) {
    ...
    const result = await consumeTicket4(auth.admin, auth.user, ticketMetaWithJob, usageId, ticketCost, corsHeaders);
    if ("response" in result) {
      return result.response;
    }
    const nextTickets = Number(result.ticketsLeft);
    ...
    const jobId2 = extractJobId4(upstreamPayload);
  ...
  if (ticketsLeft !== null && upstreamPayload && typeof upstreamPayload === "object" && !Array.isArray(upstreamPayload)) {
    upstreamPayload.ticketsLeft = ticketsLeft;
    return jsonResponse9(upstreamPayload, upstream.status, corsHeaders);
  }
  return new Response(raw, {
    status: upstream.status,
    headers: { ...corsHeaders, "Content-Type": "application/json" }
...
// ../.wrangler/tmp/pages-O1ROfj/functionsRoutes-0.8325784306386059.mjs
var routes = [
  {
